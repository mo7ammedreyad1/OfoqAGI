...
def deep_fetch(url, task):
    with sync_playwright() as p:
        ...
        # AX Tree
        ax_tree = None
        ax_tree_status = "skipped"
        try:
            ax_tree = page.accessibility.snapshot(interesting_only=True)
            ax_tree_status = "ok" if ax_tree else "null_returned"
        except Exception as ax_err:
            ax_tree_status = f"error: {ax_err}"
        import sys as _sys
        print(f"[tools.js][browser] ax_tree={ax_tree_status}", file=_sys.stderr)
